...
def update_data(data, updated_data):
    """
    This function is used to update values of data
    where keys mathing in data and updated_data.
    """
...
class UpdateJSON(View):
    ...
    def get(self, request):
        """
        This is a get request where we are rendering the update-json html template.
        """
        return render(request, 'update_json.html')

    def post(self, request):
        """
        This is a post request where,
        it accepts a action in query param
        if action == 1,
        then it accepts the json_file from frontend (request.FILES)
        and upload it to the backend.
        if action == 3,
        1. it accepts data from frontend form and merge keys and vals into a list of tuple,
        2. read the uploaded json_file and update the merge-section with the data sent from frontend.
        3. update the json_file by replacing the updated json-data.
        """
...
def download(request, path):
    """
    This function accepts a query-param sd,
     if sd == 1,
    then it send sample.json from templates for download.
    else,
    it accepts file-path as a last part of the url and send that file for the download.
    """
    try:
...
class UpdateMergeSection(View):
    ...
    def get(self, request):
        """
        This is a get request, and is responsible to render a calculate-json html template.
        """
    ...
    def post(self, request):
        ...
        where it accepts action in query-param,
        if action == 1,
        then, it accepts uploaded file from frontend and save it to the backend,
        if action == 3,
        then, it accepts script-block data from frontend and save it to the backend
        as a dynamic python script and execute it using a subprocess and accepts its
        output and set that data to the merge section to the existing uploaded json file merge section.
        """
